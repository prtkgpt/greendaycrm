  ...
  if (!customer) {
    return (
      <div className="text-center py-12">
        ...
        <Link href="/customers" className="text-green-600 hover:text-green-700 mt-2 inline-block">
          Back to customers
        </Link>
      </div>
    );
          ...
          <div className="space-y-3 text-sm">
            {customer.email && (
              <div className="flex items-center gap-2 text-gray-600">
                ...
                <a href={`mailto:${customer.email}`} className="hover:text-green-600">{customer.email}</a>
              </div>
            )}
            {customer.phone && (
              <div className="flex items-center gap-2 text-gray-600">
            ...
            <div className="divide-y divide-gray-100">
              {customer.jobs && customer.jobs.length > 0 ? (
                  ...
                  <div key={job.id} className="p-4 hover:bg-gray-50">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        {job.service_name && <p className="text-sm text-gray-500">{job.service_name}</p>}
                      ...
                      <div className="text-right">
                        ...
                        {job.price && <p className="mt-1 text-sm font-medium text-gray-900">${job.price}</p>}
                      </div>
                    </div>
                  </div>
                ))
                ...
                <div className="p-8 text-center text-gray-500">
                  <p>No jobs yet</p>
                  <Link href="/jobs" className="text-green-600 hover:text-green-700 text-sm mt-2 inline-block">Schedule a job</Link>
                </div>
              )}
            </div>
          </div>
                ...
                customer.invoices.map((invoice) => (
                  <div key={invoice.id} className="p-4 hover:bg-gray-50">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-mono text-sm text-gray-900">#{invoice.id.slice(0, 8).toUpperCase()}</p>
                        <p className="text-xs text-gray-400">{new Date(invoice.created_at).toLocaleDateString()}</p>
                      </div>
                      <div className="text-right flex items-center gap-3">
                        <StatusBadge status={invoice.status} type="invoice" />
                        <span className="font-semibold text-gray-900">${Number(invoice.amount).toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                ))
